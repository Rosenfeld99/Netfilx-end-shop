...
const CreatePro = ({ highlights, setHighlights }) => {
  const handleHighlightChange = (index, event) => {
    const updatedHighlights = [...highlights];
    updatedHighlights[index] = event.target.value;
        ...
        {highlights.map((highlight, index) => (
          <div key={index} className="flex items-center mb-2">
            <input
              ...
              value={highlight}
              onChange={(event) => handleHighlightChange(index, event)}
            />
            <button
              type="button"
